...
export default function EditMasterclass() {
  ...
  const id = params?.id || null; // Fix to prevent null error
  ...
  const [interactions, setInteractions] = useState<string>("[]"); // Default as empty JSON array
  ...
  useEffect(() => {
    ...
    const fetchMasterclass = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/masterclasses/${id}`, { credentials: "include" });
        ...
        setTopics(data.topics.length > 0 ? data.topics : [""]);

        // Fix JSON formatting for interactions
        const formattedJson =   JSON.stringify(JSON.parse(data.interactions), null, 2) // Ensures formatting with indentation
        setInteractions(formattedJson);
      } catch (err: any) {
  ...
  function handleCodeBlur() {
    try {
      ...
      setInteractions(JSON.stringify(parsed, null, 2)); // Format with indentation
      setJsonError(null);
    } catch {
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      formattedInteractions = JSON.parse(interactions);
    } catch {
    ...
    try {
      const res = await fetch(`/api/masterclasses/${id}`, {
        ...
        body: JSON.stringify({
          ...
          actionGuide,
          topics,
          interactions: formattedInteractions, // ✅ Ensure interactions is valid JSON
        }),
      });
  
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.message || "Failed to update masterclass");
      }
  
      setSuccess("Masterclass updated!");
  ...
  if (!id) return <p>Loading Masterclass ID...</p>;
  if (loading) return <p>Loading masterclass info...</p>;
...
<div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
    ...
    <form onSubmit={handleUpdate} className="flex-1">
        <h1 className="text-2xl font-bold mb-4 text-center">Edit Masterclass</h1>
